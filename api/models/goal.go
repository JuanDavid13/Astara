...
func nestTasksGoal(tasks map[int]*Task) map[int]*Task{
	for _, v := range tasks	{
		if v.Id_parent != 0 {
...
func arrayGoal(tasks map[int]*Task) []Task{
	arrayTasks := []Task{};
	for _,v :=range tasks {
...
func UpdateGoal(uid, taskId int, rol, name, deadline, dated string) bool {
  ...
  if err != nil && err != sql.ErrNoRows { /*return false;*/ panic(err);}
	...
	queryTask := "UPDATE `Goal` SET Dated = ? WHERE Id_target = ?;";
